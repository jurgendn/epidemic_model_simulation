...
map.addLayer(
  new OpenLayers.Layer.OSM(
    ...
    {
      attribution:
        "&copy; <a href='http://www.openstreetmap.org/'>OpenStreetMap</a> and contributors, under an <a href='http://www.openstreetmap.org/copyright' title='ODbL'>open license</a>. <a href='https://www.mediawiki.org/wiki/Maps'>Wikimedia's new style (beta)</a>",
...
var lonLat = new OpenLayers.LonLat(108.212, 16.068).transform(
  epsg4326,
  projectTo
);
var zoom = 7;
...
let patient = data.nodes;

// Put your point-definitions here
var markers = [];

patient.forEach((node) => {
  markers.push([node.x, node.y, node.name]);
});
...
var vectorLayer = [
  new OpenLayers.Layer.Vector(layerName[0], { styleMap: styleArray[0] }),
...
for (var i = 1; i < markers.length; i++) {
  if (!layerName.includes(markers[i][2])) {
    ...
    vectorLayer.push(
      new OpenLayers.Layer.Vector(layerName[j], {
