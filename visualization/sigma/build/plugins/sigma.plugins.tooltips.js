...
(function () {
  ...
  function Tooltips(s, renderer, options) {
    ...
    this.open = function (o, options, x, y) {
      ...
      if (options.renderer) {
        ...
        var clone = Object.create(null),
          tooltipRenderer,
          k;
        for (k in o) clone[k] = o[k];

        tooltipRenderer = options.renderer.call(
          s.graph,
          clone,
          options.template
        );
        ...
        else _tooltip.appendChild(tooltipRenderer);
      } else {
    ...
    if (options.stage) {
      if (
        options.stage.renderer !== undefined &&
      ...
      s.bind(so.show, function (event) {
        ...
        _timeoutHandle = setTimeout(function () {
          self.open(null, so, clientX, clientY);
      ...
      s.bind(so.hide, function (event) {
        var p = _tooltip;
        cancel();
        if (p) self.dispatchEvent("hidden");
      });
    ...
    if (options.node) {
      if (
        options.node.renderer !== undefined &&
      ...
      s.bind(no.show, function (event) {
        ...
        clearTimeout(_timeoutHandle);
        _timeoutHandle = setTimeout(function () {
          self.open(n, no, clientX, clientY);
    ...
    if (options.edge) {
      if (
        options.edge.renderer !== undefined &&
      ...
      s.bind(eo.show, function (event) {
        ...
        clearTimeout(_timeoutHandle);
        _timeoutHandle = setTimeout(function () {
          self.open(e, eo, clientX, clientY);
    ...
    if (no.show === "rightClickNode" || eo.show === "rightClickEdge") {
      renderer.container.addEventListener("contextmenu", contextmenuListener);
    }
  }
